...
const generateTopLeftStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
...
const generateTopStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
...
const generateTopRightStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
  if (
    rowIndex > 0 &&
...
const generateLeftStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
...
const generateRightStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
...
const generateBottomLeftStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
  if (
...
const generateBottomStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
...
const generateBottomRightStep = ({ rowIndex, columnIndex, octopuses, toIncrement }) => {
  if (
    rowIndex < octopuses.length - 1 &&
    columnIndex < octopuses[rowIndex].length - 1 &&
    octopuses[rowIndex + 1][columnIndex + 1] < 10
...
export const countFlashes = (octopusRows, step) => {
  let octopuses = octopusRows.map(octopusRow =>
    octopusRow
      .split('')
      .filter(Boolean)
      .map(octopus => Number.parseInt(octopus)),
  );
  let flashes = 0;
...
export const getStepWhenAllFlashes = octopusRows => {
  let octopuses = octopusRows.map(octopusRow =>
    octopusRow
      .split('')
      .filter(Boolean)
