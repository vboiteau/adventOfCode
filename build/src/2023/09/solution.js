const getDifferenceBetweenValues = values => {
  ...
  return [
    values[0] - nextValueDifference[0],
    ...values,
    values[values.length - 1] + nextValueDifference[nextValueDifference.length - 1],
  ];
};
export const solution01 = lines => {
  const values = lines.map(line => line.split(' ').filter(Boolean).map(Number));
  return values
    .map(getDifferenceBetweenValues)
