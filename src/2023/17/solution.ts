...
export const equals = (node1: Node, node2: Node): boolean => {
    return VectorUtils.equals(node1.vector, node2.vector) && node1.forward_count === node2.forward_count;
}
...
export const getToVisitPart1 = (node: Node, grid: Grid<number>): Array<Node> => {
    const toVisits = node.forward_count < 3 ? [forward(node), left(node), right(node)] : [left(node), right(node)];
...
export const getToVisitPart2 = (node: Node, grid: Grid<number>): Array<Node> => {
    ...
    const toVisits = node.forward_count < 10 ? [forward(node), left(node), right(node)] : [left(node), right(node)];
    return toVisits.filter(node => GridUtils.pointInGrid(grid, node.vector.point));
}

export const toKey = (node: Node): string => {
    return [node.vector.point.x, node.vector.point.y, node.vector.direction, node.forward_count].join('_');
}
...
export const isEndPart2 = (node: Node, grid: Grid<number>): boolean => {
    return PointUtils.equals(node.vector.point, GridUtils.getBottomRight(grid)) && node.forward_count >= 4;
}

export const getCityLeastLavalLost = (cityMap: Array<string>, getToVisit: (Node: Node, grid: Grid<number>) => Array<Node>, isEnd: (node: Node, grid: Grid<number>) => boolean): number => {
