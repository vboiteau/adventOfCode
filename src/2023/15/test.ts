...
file | expectedResult
${'example.txt'} | ${1320}
`('Given the list of hash in file $file when getResult is ran then return $expectedResult', async ({ file, expectedResult }) => {
...
file | expectedResult
${'example.txt'} | ${145}
`('Given the list of hash in file $file when getLensPower is ran then return $expectedResult', async ({ file, expectedResult }) => {
    const hashList = await getFileContent(join(__dirname, file));
