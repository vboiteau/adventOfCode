...
import { getApprovedSum, getProcessMap, isApproved, isCondition, isPartApproved, isPointer, isRejected, parseOperator, parsePartDescription, parseProcess, parseProcessStep, parseWorkSheet } from "./solution";
import { getFileLines } from "../../fileReader";

test(
    'Given a serialized part descriptin when parse part description is called then it should return the parsed part description.',
    () => {
...
test('Given a serialized process when parse process is called then it should return the parsed process.', () => {
    expect(parseProcess('px{a<2006:qkq,m>2090:A,rfg}')).toEqual({
        id: 'px',
        steps: [
            { type: 'condition', category: 'a', value: 2006, comparison: 'lt', ifTrue: { type: 'pointer', to: 'qkq' } },
            { type: 'condition', category: 'm', value: 2090, comparison: 'gt', ifTrue: { type: 'approved' } },
            { type: 'pointer', to: 'rfg' }
        ]
    });
});
...
test('Given a work sheet when check is approved part for each part is called then it should return the expected approval.', async () => {
    const serializedWorkSheet = await getFileLines(join(__dirname, 'example.txt'));
    const workSheet = parseWorkSheet(serializedWorkSheet);
    expect(workSheet.partDescriptions.map(part => isPartApproved(part, getProcessMap(workSheet.processes)))).toEqual([true, false, true, false, true]);
});
...
file | expected
${'example.txt'} | ${19114}
`('Given a work sheet when approved sum then it should return the expected result $expected.', async ({ file, expected }) => {
