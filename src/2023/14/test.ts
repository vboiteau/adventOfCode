...
file | expectedLoad
${'example.txt'} | ${136}
`('Given a file $file describing a panel when calculating the load to tilt it then the load should be $expedLoad', async ({ file, expectedLoad }) => {
...
`('Given a file $file describing a panel when calculating the load to tilt after $cycles cycles then the load should be $expedLoad', async ({ file, cycles, expectedLoad }) => {
    const panel = await getFileLines(join(__dirname, file));
