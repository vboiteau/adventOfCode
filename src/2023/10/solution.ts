...
const getLinkedPoints = (
  mapInstructions: Array<Array<MapInstruction>>,
  point: Point,
): Array<Point> => {
...
export const solution01 = (input: Array<string>): number => {
  const mapInstructions: Array<Array<MapInstruction>> = input.map(line =>
    line.split('').map(parseMapInteraction),
  );
