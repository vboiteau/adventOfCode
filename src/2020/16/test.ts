...
import { getOutOfRangeFieldsSum, getMyTicket, getProductOfFieldStartingWith } from './solution';
...
`(
  ...
  async ({ file, sum }) => {
    const ticketNotes: string = await getFileContent(resolve(join(__dirname, file)));
...
`(
  ...
  async ({ file, startWith, product }) => {
    const ticketNotes: string = await getFileContent(resolve(join(__dirname, file)));
    expect(getProductOfFieldStartingWith(ticketNotes, startWith)).toEqual(product);
  }
); 
