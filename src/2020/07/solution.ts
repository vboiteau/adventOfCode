function getBagMap(bagLines) {
  return bagLines.reduce((map, bagLine) => {
    ...
    const canContain = (bagLine.match(/\d\s\w+\s\w+/g) || []).map(withNumber => {
      const { count, type } = withNumber.match(/(?<count>\d)\s(?<type>\w+\s\w+)/).groups;
      return {
...
export function countBagsCanContain(bagLines, targetBagType): Test {
  ...
  let bagsToCheck = [targetBagType];
  let bagCount = 0;
  for (var i = 0; i < bagsToCheck.length; i++) {
...
export function getCountForBag(currentCount, bagMap, targetBagType) {
  ...
  return currentCount + canContain.reduce(
    (sum, { type, count }) => sum + getCountForBag(currentCount * count, bagMap, type),
    0
  );
}
