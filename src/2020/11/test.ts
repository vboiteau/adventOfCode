import { getOccupiedSeatAtTheEnd, SeatState, countOccupiedSeatAround, countOccupiedNextSeat } from './solution';
...
`(
  ...
  async ({ file, occupiedSeats, counter, maxOccupied }) => {
    ...
    const seatPlan: Array<Array<SeatState>> = (await getFileLines(seatPlanFile)).map((seatRow: string) => seatRow.split(''));
    expect(getOccupiedSeatAtTheEnd(seatPlan, counter, maxOccupied)).toBe(occupiedSeats);
  }
);
