export function getProductOfBusNumberAndTimeToWait(notesFile: string): number {
  ...
  const buses: Array<number> = notes[1].split(',').filter(bus => bus !== 'x').map(bus => parseInt(bus));
  const sortByCloseToTimeReady = buses.sort((a, b) => (a - (timeReady % a)) - (b - (timeReady % b)));
  return (sortByCloseToTimeReady[0] - (timeReady % sortByCloseToTimeReady[0])) * sortByCloseToTimeReady[0]; 
}
...
export function getTimestampWithSequence(buses: Array<string>): number {
  const orderedBuses: Array<OrderedBus> = buses
    .map((bus, index) => bus !== 'x' ? { index: index, id: parseInt(bus) } : false)
    .filter(minuteFromFirst => minuteFromFirst !== false);
  return crt(orderedBuses.map(({id}) => id), orderedBuses.map(({id, index}) => id - index));
}
