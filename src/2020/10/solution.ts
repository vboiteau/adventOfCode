export function getDifferentialProduct(adapters: Array<number>): number {
  ...
  const differential: Array<number> = joltages
    .reduce((diff: Array<number>, joltage: number, index: number) => {
      if (index > 0) {
...
export function getDistinctArrangements(adapters: Array<number>): number {
  const joltages: Array<number> = buildJoltagesList(adapters);
  return joltages.reduce((arrangements: Array<number>, joltage: number, index: number) => {
    if (index > 0) {
      const previousJoltage: number = joltages[index - 1];
      arrangements = branchArrangementsWithJump(joltages, joltage, previousJoltage, arrangements, 1);
      arrangements = branchArrangementsWithJump(joltages, joltage, previousJoltage, arrangements, 2);
      arrangements[joltage] += arrangements[previousJoltage];
    } else {
      arrangements[0] = 1;
    }
    return arrangements;
  }, new Array(joltages[joltages.length - 1] + 1).fill(0)).pop();
}
