...
export function getCumulativeMemoryValue(instructions: Array<string>): number {
  const maskInstructions: Array<MaskInstruction> = parseInstructions(instructions);
  const memory: Array<number> = convertMaskInstructionsToMemory(maskInstructions);
  return memory.reduce((sum, value) => sum + (value || 0), 0);
}

export function getCumulativeMemoryValueWithMaskedAddress(instructions: Array<string>): number {
  const maskInstructions: Array<MaskInstruction> = parseInstructions(instructions);
  const memory: Array<number> = convertMaskInstructionsToMemoryWithMaskedAddress(maskInstructions);
  return memory.reduce((sum, value) => sum + (value || 0), 0);
}

function parseInstructions(instructions: Array<string>): Array<MaskInstruction> {
  return instructions.reduce((parsed, line) => {
    ...
    if (line.startsWith("mem")) {
     const {
       address,
       value
     } = line.match(/^mem\[(?<address>\d+)\]\s=\s(?<value>\d+)$/).groups;
...
function convertMaskInstructionsToMemory(maskInstructions: Array<MaskInstruction>): Array<number> {
  return maskInstructions.reduce((written: Array<number>, maskInstruction: MaskInstruction) => {
    maskInstruction
      .instructions
...
function convertMaskInstructionsToMemoryWithMaskedAddress(maskInstructions: Array<MaskInstruction>): Array<number> {
  const memory = maskInstructions.reduce((written: Map<number, number>, maskInstruction: MaskInstruction) => {
    maskInstruction
      .instructions
      .forEach(({ value, address }) => {
        const maskedAddress: string = getMaskedAddress(maskInstruction.mask, address);
...
function explodeMaskedAddress(maskedAddress) {
  return maskedAddress.split('X').reduce((addresses, nextPart) => {
