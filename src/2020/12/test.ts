import { getLastPosition, getLastPositionWithWaypoint, getMarithanPosition } from './solution';
...
`(
  ...
  async ({ file, marithanPosition, positionCalculator }) => {
    const instructionsFile = resolve(join(__dirname, file));
    const instructions: Array<string> = (await getFileLines(instructionsFile));
    expect(getMarithanPosition(instructions, positionCalculator)).toEqual(marithanPosition);
  }
);
